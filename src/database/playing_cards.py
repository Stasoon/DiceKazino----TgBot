...
from .models import BlackjackPlayerCard
...
async def add_card_to_player_hand(
    ...
    await BlackjackPlayerCard.create(
        game_id=game_number,
...
async def get_player_cards(game_number: int, player_id: int) -> list[BlackjackPlayerCard]:
    moves = await BlackjackPlayerCard.filter(game_id=game_number, player_id=player_id).all()
    return moves
...
async def count_player_score(game_number: int, player_id: int) -> int:
    score = await BlackjackPlayerCard.filter(game_id=game_number, player_id=player_id).annotate(
        score=Sum('points')
    ).values_list('score')
    return score[0][0]
...
async def delete_player_cards(game_number: int, user_id: int) -> None:
    try:
        await BlackjackPlayerCard.filter(game_id=game_number, player_id=user_id).delete()
    except DoesNotExist:
        pass
...
async def delete_game_cards(game_number: int) -> None:
    try:
        await BlackjackPlayerCard.filter(game_id=game_number).delete()
    except DoesNotExist:
        pass
