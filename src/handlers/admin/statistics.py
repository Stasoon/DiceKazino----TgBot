...
async def get_stats_image(days_back: int):
    ...
    axs[1].pie(
        deposit_method_frequency.values(), labels=[m.value or 'Админ' for m in deposit_method_frequency.keys()],
        autopct='%1.1f%%', startangle=140
    )
    axs[1].set_title('Методы пополнения')
    # axs[1].set_title('Частота использования методов платежа')

    axs[2].pie(
        withdraw_method_frequency.values(), labels=[m.value or 'Админ' for m in withdraw_method_frequency.keys()],
        autopct='%1.1f%%', startangle=140
    )
...
async def __handle_show_transactions_stats(message: Message):
    ...
    for i in s:
        text = "\n\n\n".join([await get_stats(model=model, days_back=i) for model in m])
