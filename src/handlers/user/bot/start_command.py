...
async def handle_start_cmd_with_referral_link(message: Message, command: CommandObject, created_user):
...
async def handle_empty_start_cmd(message: Message):
    ...
    # создаём пользователя, если не существует
    await users.create_user_if_not_exists(
...
def register_start_command_handler(router: Router):
    ...
    # Запуск бота по реферальной ссылке
    router.message.register(
        handle_start_cmd_with_referral_link, CommandStart(deep_link=True, magic=F.args.startswith('ref'))
    )
