...
async def send_cryptobot_invoice(message: Message, currency: str, deposit_amount: float):
    ...
    if not pay_url or not invoice_id:
        await message.answer(ExceptionMessages.get_payment_error(), parse_mode='HTML')
        return

    await message.answer(
        UserMessages.get_deposit_link_message(),
        reply_markup=PrivateKeyboards.get_invoice(PaymentMethod.CRYPTO_BOT, pay_url, invoice_id),
        parse_mode='HTML')
...
async def handle_deposit_callback(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        text=UserMessages.get_choose_deposit_method(),
        reply_markup=PrivateKeyboards.get_payment_methods(transaction_type=TransactionType.DEPOSIT),
        parse_mode='HTML'
    )
...
async def handle_show_deposit_method_callbacks(callback: CallbackQuery, callback_data: BalanceTransactionCallback,
    ...
    # если метод вывода - полуавтоматический
        ...
        await callback.message.answer(
            text=UserMessages.enter_deposit_amount(min_deposit_amount=Config.Payments.min_deposit_amount),
...
async def handle_halfauto_deposit_amount_message(message: Message, state: FSMContext):
    ...
    if deposit_amount:
        ...
        await message.answer(
            text=UserMessages.get_half_auto_deposit_method_requisites(deposit_method),
            parse_mode='HTML')
...
async def handle_halfauto_deposit_screen_message(message: Message, state: FSMContext):
    ...
    if not message.photo:
        await message.answer(ExceptionMessages.get_photo_expected(), parse_mode='HTML')
        return
    ...
    try:
        ...
        await message.answer(UserMessages.get_wait_for_administration_confirm(),
...
async def handle_cryptobot_deposit_currency_callback(callback: CallbackQuery, callback_data: BalanceTransactionCallback,
    ...
    await callback.message.delete()
    await callback.message.answer(
        text=UserMessages.enter_deposit_amount(Config.Payments.min_deposit_amount),
        reply_markup=PrivateKeyboards.get_cancel_payment(),
        parse_mode='HTML'
    )
...
async def handle_check_cryptobot_payment(callback: CallbackQuery, callback_data: PaymentCheckCallback):
    ...
    if invoice.get('status') != 'paid':
        await callback.answer(ExceptionMessages.get_payment_not_found())
        return
    ...
    await callback.message.delete()
    await callback.message.answer(
        text=UserMessages.get_deposit_confirmed(),
        reply_markup=PrivateKeyboards.get_main_menu(),
        parse_mode='HTML'
    )
