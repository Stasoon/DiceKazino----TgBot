...
from src.messages import UserMessages, ExceptionMessages
...
async def validate_and_get_transaction_amount(amount_message: Message) -> float | None:
    try:
        ...
        await amount_message.answer(ExceptionMessages.get_invalid_number_message(), parse_mode='HTML')
        return None
    ...
    if transaction_amount > user_balance:
        await amount_message.answer(
            text=ExceptionMessages.get_insufficient_balance(user_balance),
            parse_mode='HTML')
        return None
    elif transaction_amount < min_transaction_amount:
        await amount_message.answer(
            text=ExceptionMessages.get_insufficient_transaction_amount(min_transaction_amount)
        )
...
async def handle_withdraw_callback(callback: CallbackQuery):
    ...
    if await users.get_user_balance(callback.from_user.id) < Config.Payments.min_deposit_amount:
        await callback.answer(
            text=ExceptionMessages.low_balance_for_withdraw(Config.Payments.min_withdraw_amount),
            show_alert=True)
        return

    await callback.message.edit_text(
        text=UserMessages.get_choose_withdraw_method(),
        reply_markup=PrivateKeyboards.get_payment_methods(transaction_type=TransactionType.WITHDRAW),
        parse_mode='HTML')
...
async def handle_show_withdraw_method_callbacks(callback: CallbackQuery, callback_data: BalanceTransactionCallback,
    ...
    if callback_data.method == PaymentMethod.CRYPTO_BOT:
        await callback.message.edit_text(
            text=UserMessages.choose_currency(),
            reply_markup=await PrivateKeyboards.get_cryptobot_choose_currency(callback_data.transaction_type),
            parse_mode='HTML'
        )
    # если метод вывода полуавтоматический
        ...
        await callback.message.answer(
            text=UserMessages.enter_withdraw_amount(min_withdraw_amount=Config.Payments.min_withdraw_amount),
            reply_markup=PrivateKeyboards.get_cancel_payment(), parse_mode='HTML'
        )
...
async def handle_withdraw_amount_message(message: Message, state: FSMContext):
    ...
    if withdraw_amount:
        await message.answer(UserMessages.enter_user_withdraw_requisites(data.get('method')),
                             parse_mode='HTML')
...
async def handle_user_withdraw_requisites(message: Message, state: FSMContext):
    if not message.text:
        await message.answer(ExceptionMessages.get_text_expected(), parse_mode='HTML')
        return
    elif len(message.text) > 80:
        await message.answer(ExceptionMessages.get_message_text_too_long(), parse_mode='HTML')
        return
    elif len(message.text) < 8:
        await message.answer(ExceptionMessages.get_message_text_too_short(), parse_mode='HTML')
        return

    await message.answer(UserMessages.get_confirm_withdraw_requisites(),
                         reply_markup=PrivateKeyboards.get_confirm_withdraw_requisites(),
                         parse_mode='HTML')
...
async def handle_confirm_withdraw_callback(callback: CallbackQuery, callback_data: ConfirmWithdrawRequisitesCallback,
    ...
    if not callback_data.requisites_correct:
        await callback.message.edit_text(
            UserMessages.enter_user_withdraw_requisites(data.get('method')), parse_mode='HTML')
        await state.set_state(UserStates.HalfAutoWithdraw.wait_for_requisites)
        return
    ...
    await callback.message.delete()
    await callback.message.answer(UserMessages.get_wait_for_administration_confirm(),
